...
# VPN, Tor, AdGuard Home, Dynamic DNS and AstroWarp pages in the admin panel.
...
cmd_status() {
    ...
    _cc=$(read_proc_cc)
    if [ "$_cc" = "CN" ]; then
        warn "Region is CN - the VPN, Tor, AdGuard Home, Dynamic DNS and"
        warn "AstroWarp pages are hidden in the admin panel."
    else
...
cmd_backup() {
    ...
    echo
    echo "    scp -O root@$(lan_ip):$_out ~/glinet-factory-backup.bin"
    echo
    echo "Then confirm the copy is intact:"
    echo "    md5 -q ~/glinet-factory-backup.bin        # macOS"
    echo "    md5sum ~/glinet-factory-backup.bin        # Linux / WSL"
    echo
...
cmd_restore() {
    ...
    sync
    green "restored - country code now [$(read_flash_cc)]"
    warn "reboot now"
}
...
Run this ON the router, as root, over SSH.
  ...
  $0 status              Show model, firmware, Factory partition, region code and
                         Wi-Fi regulatory domain. Read-only. Always run this first.

  $0 backup [file]       Dump the Factory partition (default: $DEFAULT_BACKUP).
                         Does not modify flash. Do this before patching.
  ...
  $0 restore [backup]    Write a backup image back to the Factory partition.
